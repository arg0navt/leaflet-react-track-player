...
class LeafletReactTrackPlayer extends MapLayer {
  ...
  updateLeafletElement(fromProps, toProps) {
    if (fromProps.track.length !== toProps.track.length) {
      ...
      const values = Object.values(newPointsPolyline._layers);
      this.leafletElement.snakePolyline.addPolyline(
        values,
        newPointsPolyline._detailData
      );
      const distanceNewPoints = newPointsPolyline._originalLatlngs.reduce(
        (result, item, index) => {
          var point = L.latLng(item);
          if (newPointsPolyline._originalLatlngs[index + 1]) {
            result =
              result +
              point.distanceTo(newPointsPolyline._originalLatlngs[index + 1]);
          }
          return result;
        },
        0
      ...
      if (this.props.progressFormat === "default") {
        const activeIndex = Number(
          (
            ((fromProps.track.length - 1) / 100) *
            this.state.activePosition
          ).toFixed()
        );
        this.setState({
          activePosition: (activeIndex / (toProps.track.length - 1)) * 100,
          track: toProps.track,
          maxDistance: this.state.maxDistance + distanceNewPoints
        });
        ...
        this.setState(
          {
            ...
            durationTrack:
              moment(
                toProps.track[toProps.track.length - 1].t,
                toProps.timeFormat
              ) - moment(toProps.track[0].t, toProps.timeFormat)
          },
          () => {
            const itemDuration =
              moment(this.state.activeTimeStamp, this.props.timeFormat) -
              moment(toProps.track[0].t, this.props.timeFormat);
              this.setState({
      ...
      <Control position="bottomleft">
        <div className="leaflet-control leaflet-react-track-player">
          <div className="tp-buttons">
            ...
            />
            <button className="tp_button stop" onClick={() => this.stop()} />
            <button
