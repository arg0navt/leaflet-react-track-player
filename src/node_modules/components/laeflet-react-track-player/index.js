...
class LeafletReactTrackPlayer extends MapControl {
    ...
    componentDidMount() {
        ...
        const maxDistance =  this.props.track.reduce((result, item, index) => {
            if (index > 0) {
                const currPoint = this.props.leaflet.map.latLngToContainerPoint(this.props.track[index - 1]);
                const nextPoint = this.props.leaflet.map.latLngToContainerPoint(item);
        ...
        this.leafletElement.snakePolyline.snakeIn(
            (e, d) => {
                progressDistance = d + dPrev;
                this.leafletElement.finishMarker.setLatLng(e);
                this.setState({activePosition: progressDistance / (maxDistance / 100)})
            }, 
            (point) => {
                this.leafletElement.finishMarker.setIcon(createIcon(point.course));
                dPrev = progressDistance;
                // console.log(progressDistance, maxDistance)
            },
