...
class LeafletReactTrackPlayer extends MapControl {
    ...
    componentDidMount() {
        this.leafletElement.snakePolyline.snakeIn(
            (e, d) => {
                if (e) this.leafletElement.finishMarker.setLatLng(e);
                if (d !== null) this.setState({activePosition: d});
            }, 
            (point) => this.leafletElement.finishMarker.setIcon(this.createIcon(point.course)),
            () => this.setState({active: false}),
            (point, position) => {
                if (point) this.leafletElement.finishMarker.setLatLng(point);
                if (position !== null) this.setState({activePosition: position});
            }
        )
    }
    ...
    changeActivePosition = (e) => {
        ...
        const activePositionTime = moment(this.props.track[0].t, "YYMMDDHHmmss000").add((durationTrack / 100) * activePosition, "millisecond");
