...
L.Polyline.include({
	...
	newPosition: function () {
		this._latlngs.push([51.999775, 81.7681916666666])
	},
...
L.LayerGroup.include({
	...
	newPosition: function(time) {
		const n = this._originalLatlngs.filter((item) => {
			if (item.t <= time) {
				return item;
			}
		});
		...
		this._snakingLayers.map((item) => {
			if (ln > 0) {
				if (item._map) {
					if (ln - item._snakeLatLngs[0].length >= 0) {
						item.setPosition(item._snakeLatLngs[0])
						ln = ln - item._snakeLatLngs[0].length;
					} else {
						const needPoints = item._snakeLatLngs[0].filter((item, index) => index <= ln)
						item.setPosition(needPoints)
						ln = ln - item._snakeLatLngs[0].length;
					}
				} else {
					if (ln - item._latlngs.length >= 0) {
						item.setPosition(item._latlngs)
						ln = ln - item._latlngs.length;
					} else {
						const needPoints = item._latlngs.filter((item, index) => index <= ln)
						item.setPosition(needPoints)
						ln = ln - item._latlngs.length;
					}
				}
