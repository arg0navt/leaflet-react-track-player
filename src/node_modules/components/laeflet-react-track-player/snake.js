	...
	initDefaultPosition: function (params) {
		...
		if (params.last) {
			for (var i in this._eventParents) {
				this._eventParents[i]._takeActivePoint(params.points[params.points.length - 1]);
			}
		}
		if (!this._snakeLatLngs) this._snakeLatLngs = this._latlngs;
...
L.LayerGroup.include({
	...
	changePositionTime: function (value) {
		...
		this._detailData.reduce((result, item, index) => {
			...
			if (result.count > 0 && result.count <= item.length) {
				const filterData = item.filter((itm, ind) => ind <= result.count - 1);
				result.data.push(filterData)
				result.last = index;
				result.count = result.count - filterData.length;
			}
			return result;
