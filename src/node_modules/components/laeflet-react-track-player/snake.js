...
L.Polyline.include({
	...
	initDefaultPosition: function (params) {
		...
		this._play = false;
		this._snakingTime = 0;
		this._snakingVertices = params.points.length !== 1 ? params.points.length - 1 : 0;
		this._snakingDistance = params.points.reduce((result, item, index) => {
			if (index > 0) {
				const currPoint = this._map.latLngToContainerPoint(params.points[index - 1]);
		...
		if (this._snakeLatLngs.length === params.points.length) {
			...
			} else this._latlngs = [this._snakeLatLngs[0], this._snakeLatLngs[0]];
		}
	},
