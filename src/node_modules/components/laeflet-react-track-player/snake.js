...
L.LayerGroup.include({
      ...
      (result, item, index) => {
        if (result.count > item) {
          ...
          if (result.count > 0 && result.count <= item) {
            ...
            result.max = result.max + prevDistance;
            result.data.push(d);
            result.last = index;
  ...
  changePosition: function(value) {
    ...
    let distance = this._options.progressFormat === "distance" ? filterData.max : null;
    this._callbackChangePosition(last[last.length - 1], distance);
    this._end(false);
  },
  ...
  snakeIn: function(events) {
    ...
    this._detailDistance = this._detailData.map(polyline => {
      return polyline.reduce((result, point, index) => {
        if (polyline[index + 1]) {
          result = result + L.latLng(point).distanceTo(polyline[index + 1]);
        }
        return result;
      }, 0);
    });
