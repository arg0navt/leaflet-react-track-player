...
L.Polyline.include({
	...
	_snake: function () {
		...
		if (this._play) {
			let time = this._timeDistance ? this._timeDistance / this._xSpeed : 1;
			let diff = this._now - this._snakingTime;
			let forward = this.maxDistance ? diff * (this.maxDistance / time) / 1000 :  diff * (1 / time) / 1000;
	...
	_snakeForward: function (forward) {
		...
		if (this._play && this._map) {
			...
			let percent = this.maxDistance ? this._snakingDistance / this.maxDistance : 0;
	...
	_nextPoint: function () {
		for (let i in this._eventParents) {
			let countLayer = this._eventParents[i]._snakingLayersDone - 1;
			let point = this._eventParents[i]._detailData[countLayer][this._snakingVertices];
			if (this._eventParents[i]._options.playerAndTime) {
				...
				let count = this._eventParents[i]._detailData.reduce((result, item, index) => {
					if(index < countLayer) {
