...
L.Polyline.include({
	...
	initDefaultPosition: function (animate, last, points) {
		this.latLngAnimation = animate;
		this._snaking = last ? true : false;
		this._play = false;
		this._snakingTime = 0;
		this._snakingVertices = points.length;
		this._snakingDistance = points.reduce((result, item, index) => {
			if (index > 1) {
				const currPoint = this._map.latLngToContainerPoint(points[index - 1]);
...
L.LayerGroup.include({
	...
	changePosition: function (time) {
		...
		filterData.data.map((item, index) => {
			...
			this._snakingLayers[index].initDefaultPosition(this.latLngAnimation, index === filterData.last, item);
		})
