...
L.LayerGroup.include({
	...
	changePosition: function (time) {
		...
		this._detailData.map((item, index) => {
			filterData = item.filter((itm) => Number(itm.t) <= Number(time));
		...
		activeRange = filterData.length >= 2 ? filterData.reduce((result, item, index) => {
			if (index > 0) {
				const currPoint = this._map.latLngToContainerPoint(filterData[index - 1]);
				const nextPoint = this._map.latLngToContainerPoint(item);
				result = result + currPoint.distanceTo(nextPoint);
			}
			return result;
		}, 0) : 0;
		this.callbackChangePosition(filterData[filterData.length - 1], (activeRange + prevRange) / (this._maxDistance / 100));
		this.end();
	},
		...
		if (!this.latLngAnimation && e) {
			...
			this.latLngAnimation = function (point, progress) {
				if (progress) {
					...
					console.log(prevRangeColors, activePolylineRange, progress.distance, progress.n)
