...
L.LayerGroup.include({
	...
	changePosition: function (time) {
		const filterData = this._detailData.reduce((result, item, index) => {
			var filterData = item.filter((itm) => Number(itm.t, this._options.timeFormat) <= Number(time, this._options.timeFormat));
		...
		this.callbackChangePosition(last[last.length - 1]);
		this.end(false);
	},
	...
	snakeIn: function (events) {
		...
		if (!this.latLngAnimation && events.fly) {
			this.latLngAnimation = function (point) {
				events.fly(point)
			}
		}
		if (!this.takeActivePoint && events.nextPoint) this.takeActivePoint = events.nextPoint;
