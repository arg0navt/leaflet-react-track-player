...
L.Polyline.include({
	...
	changePosition: function (items) {
		if (items.length > 1) {
			...
			if (items.length === this._snakeLatLngs[0].length) {
				this._snakeEnd()
			}
		}
	},
...
L.LayerGroup.include({
	...
	changePosition: function (time) {
		...
		this._detailData.map((item, index) => {
			const filterData = item.filter((itm) => Number(itm.t) <= Number(time));
			if ((filterData.length !== item.length || !filterData.length) && n === null) {
				n = index + 1;
			}
			if (!this._snakingLayers[index]._map && filterData.length >= 1) {
				...
				this.latLngAnimation(filterData[filterData.length - 1])
				this.end()
			}
			if (this._snakingLayers[index]._map && !filterData.length) this.removeLayer(this._snakingLayers[index])
			this._snakingLayers[index].changePosition(filterData)
		})
		this._snakingLayersDone = n;
	},

	snakeIn: function (e, n, end) {
		if (!this.latLngAnimation && e) {
			...
			this.latLngAnimation = function (point, progress) {
				if (self._snakingLayersDone > 1) {
					prevRange = self._detailDistance.reduce((result, range, index) => {
						if (index < self._snakingLayersDone - 1) {
							result = result + range;
						}
						return result;
					}, 0);
				}
				if (prevData !== null && (prevData.n !== progress.n)) {
					if (progress.n < prevData.n) {
						activePolylineRange = 0
					} else activePolylineRange = activePolylineRange + prevData.distance;
				}
				resultDistance = prevRange + activePolylineRange + progress.distance;
				prevData = progress;
